...
(function(root, factory) {
	...
	$.Dialog = function(settings) {
		if (settings.type === "alert") {
			...
			var htmla = '<div class="ui-alert-title">' + settings.content + '</div>';
			var action = '';
			if (settings.button) {
		...
		if (settings.type === "confirm") {
			var dialog = new Dialog();
			var htmla = '<div class="ui-confirm-title">' + settings.content + '</div>';
			var action = '';
			if (!settings.buttons) {
				settings.buttons = [{
					'yes': '确定'
			...
			for (var i = 0, l = settings.buttons.length; i < l; i++) {
				...
				if (item.yes) {
					btnstr += '<td width="30%"><button class="ui-confirm-submit " data-type="yes">' + item.yes + '</button></td>';
				}
				if (item.no) {
					btnstr += '<td width="40%">&nbsp;</td><td><button class="ui-confirm-no" data-type="no">' + item.no + '</button></td>';
				}
				if (item.close) {
					btnstr += '<td width="30%"><button class="ui-confirm-close js-dialog-close" data-type="close">' + item.close + '</button></td>';
				}
			}
	...
	*/
	...
	Dialog.prototype = {
		...
		show: function() {
			...
			this.height = this.settings.height || '180px' //this.dialogContainer.height();
