		...
		$.ajax({
            ...
            $('.xzspan').click(function(evt){
                ...
                if(!$(this).hasClass('xzspan33')){
                    ...
                    var seatsInGroup = _seatMap[seatId].SeatsInGroup;
                    if(seatsInGroup != null){
                    	for(var s in seatsInGroup){
                    		var seatRowId = seatsInGroup[s].RowIndex>10?''+seatsInGroup[s].RowIndex:'0'+seatsInGroup[s].RowIndex;
                    		var seatColId = seatsInGroup[s].ColumnIndex>10?''+seatsInGroup[s].ColumnIndex:'0'+seatsInGroup[s].ColumnIndex;
                    		var sid = seatRowId+seatColId;
                    		addSeat(sid, $('#sd'+sid));
                    	}	
                    }else{
                    	addSeat(seatId, $(this));
		...
		for(var i=0;i<_rowCount;i++){
			...
			for(var j=0;j<_ColumnCount;j++){
				...
				if(_seatMap[seatId] != null){
					...
					//see if have groupSeat
					var groups = _seatMap[seatId].SeatsInGroup;
					if(groups != null){
						for(var k=0;k<groups.length;k++){
							if(groups[k].ColumnIndex==j && k == 0){
								columnHtml.addClass('xzspanGROUPLeft');
								break;
							}else{
								columnHtml.addClass('xzspanGROUPRight');
								break;
							}
						}						
					}
	...
	function updatePrice(){
		var price = 0;
		if(_paymentMethod=='wechat' && _memberLevelId<=1){
	...
	function addOrder(){
		if(_memberLevelId<0 || _memberLevelId == ""){
			showReg();
			return
		}
		var filmOrder = {};
